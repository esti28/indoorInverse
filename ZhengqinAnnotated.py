...
# 1st part of model - Iniitializing hyperparameters
...
# Fine Tune - you have a pretrained model and start with pretrained weights
...
# The training weight
# definition: relative weight of the losses
# Balances the weights so network does not put too much emphasis on one modality over the other
# (ex: prioritizing the albedo loss too much may cause depth loss to grow)
...
# Think of opt like a dictionary of parameters
opt = parser.parse_args()
print(opt)

#hyperparameters
opt.gpuId = opt.deviceIds[0] # opt = options dictionary storing parameters
...
# Initial Network
# Corresponds to the green blobs seen in the supplmentary paper of Zhengqin's [Figure 7]
